...
public class signUpTest {
    ...
    public void setupReport() {
        String reportPath = Paths.get("reports", "signUpAPITestReport.html").toString();
 ...
 // --- Utility: Read JSON file as String ---
    ...
    public void testSignup() throws InterruptedException {
        String requestBody = readJsonFromFile("signup.json");

        String reqImagePath = saveTextAsImage(requestBody, "Signup_Request");
        ...
        try {
            ...
            String resImagePath = saveTextAsImage(response.asPrettyString(), "Signup_Response");
            if (resImagePath != null) test.info("Response JSON").addScreenCaptureFromPath(resImagePath);

            test.pass("Signup API passed with status code: " + response.statusCode());

        } catch (AssertionError ae) {
            test.fail("Assertion failed: " + ae.getMessage());
            throw ae;
        } catch (Exception e) {
            test.fail("Signup API execution failed: " + e.getMessage());
            throw e;
        }
        System.out.println("signup done check mail");
        Thread.sleep(5000);
    }
